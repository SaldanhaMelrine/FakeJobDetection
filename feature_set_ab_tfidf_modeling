...
df = pd.read_csv(r"C:/Users/dell/Downloads/Final_Job_Posting.csv")
...
# Features
categorical = ['required_experience', 'required_education', 'company', 'company_size_bin']
numeric_binary = ['telecommuting', 'has_company_logo', 'has_questions', 'has_website', 'year', 'Review', 'salary_avg', 'salary_deviation']
linguistic_features = ['readability', 'sentiment_polarity', 'sentiment_subjectivity', 'capitalization_ratio', 'punctuation_count', 'special_char_count']
structural_features = [
...
other_features = categorical + numeric_binary + linguistic_features + structural_features
X_other = df[other_features].astype(float).reset_index(drop=True)
X_all = hstack([csr_matrix(X_other.values), tfidf_matrix])
features_all = other_features + tfidf_feature_names
y = df['fraudulent'].astype(int)

# Train-test split
X_train, X_test, y_train, y_test = train_test_split(X_all, y, stratify=y, random_state=42, test_size=0.2)
...
for name, (model, params) in models.items():
    ...
    if hasattr(model, "feature_importances_"):
        ...
        for idx in indices:
            print(f"{features_all[idx]}: {importances[idx]:.4f}")
    elif hasattr(model, "coef_"):
        coefs = model.coef_[0]
        indices = np.argsort(np.abs(coefs))[::-1][:top_n]
        for idx in indices:
            print(f"{features_all[idx]}: {coefs[idx]:.4f}")
